import numpy as np
import matplotlib.pyplot as plt  # Pra espiral depois

phi = (1 + np.sqrt(5)) / 2  # 1.618 magic!

def aureo_state(subcamada=1, ruido=0.0):
    state = np.array([1/np.sqrt(2)+0j, 1/np.sqrt(2)+0j])  # Force complex dtype
                                                ...
                                                if subcamada > 1:  # Entanglement via kron
                                                        state2 = state.copy()
                                                                state = np.kron(state, state2)
                                                                    ...
                                                                    if subcamada == 1:
                                                                            probs = (np.abs(state)**2)[:2] # Fix: added parentheses around np.abs(state)**2
                                                                                else:
                                                                                        probs = np.array([np.sum(np.abs(state[0::2])**2), np.sum(np.abs(state[1::2])**2)])

                                                                                            probs /= np.sum(probs)  # Perfect normalization
                                                                                                return probs

                                                                                                # Quick test – deve sair sem erro!
